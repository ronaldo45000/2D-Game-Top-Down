class Mantis {
    constructor(game, x, y, path) {
        ...
        this.spritesheet = ASSET_MANAGER.getAsset("./sprites/mantis.png");
        this.targetID = 0;
    ...
    loadAnimations(){
        ...
        //walking
        this.animations[0][DirectionBoar.DOWN]= new Animator(this.spritesheet, 0, 0, 64, 64, 3, 0.15, 0, false, true);
        this.animations[0][DirectionBoar.LEFT]= new Animator(this.spritesheet, 0, 64, 64, 64, 3, 0.15, 0, false, true);
        this.animations[0][DirectionBoar.RIGHT]= new Animator(this.spritesheet, 0, 64*2, 64, 64, 3, 0.15, 0, false, true);
        this.animations[0][DirectionBoar.UP]= new Animator(this.spritesheet, 0, 64*3, 64, 64, 3, 0.15, 0, false, true);
  
        //attacking
        this.animations[1][DirectionBoar.DOWN]= new Animator(this.spritesheet, 388, 0, 29, 27, 4, 0.2, 0, false, true);
        this.animations[1][DirectionBoar.LEFT]= new Animator(this.spritesheet, 388, 29, 29, 27, 4, 0.2, 0, false, true);
        this.animations[1][DirectionBoar.RIGHT]= new Animator(this.spritesheet, 388, 29*2, 27, 26, 4, 0.2, 0, false, true);
        this.animations[1][DirectionBoar.UP]= new Animator(this.spritesheet, 388, 29*3, 29, 27, 4, 0.2, 0, false, true);
  
        //Idling
        this.animations[2][DirectionBoar.DOWN]= new Animator(this.spritesheet, 192, 0, 64, 64, 3, 0.15, 0, false, true);
        this.animations[2][DirectionBoar.LEFT]= new Animator(this.spritesheet, 192, 64, 64, 64, 3, 0.15, 0, false, true);
        this.animations[2][DirectionBoar.RIGHT]= new Animator(this.spritesheet, 192, 64*2, 64, 64, 3, 0.15, 0, false, true);
        this.animations[2][DirectionBoar.UP]= new Animator(this.spritesheet, 192, 64*3, 64, 64, 3, 0.15, 0, false, true);
    ...
    updateBB() {
      ...
      this.BB = new BoundingBox(this.x - this.game.camera.x - (this.width), this.y - this.game.camera.y - (this.height), 2*this.width, 2*this.height);
    ...
    draw(ctx) {
        ...
        if(this.state == 1){
            if(this.facing == DirectionBoar.RIGHT ){
                this.animations[2][this.facing].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - (this.width),this.y - this.game.camera.y - (this.height) ,2*PARAMS.SCALE);

                this.animations[1][DirectionBoar.RIGHT].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x,this.y - this.game.camera.y,2*PARAMS.SCALE);
            }
          
            else if(this.facing == DirectionBoar.LEFT){
                this.animations[2][this.facing].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - (this.width),this.y - this.game.camera.y - (this.height) ,2*PARAMS.SCALE);

                this.animations[1][DirectionBoar.LEFT].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - 35,this.y - this.game.camera.y,2*PARAMS.SCALE);
            }
        
            else if(this.facing == DirectionBoar.UP){
                this.animations[1][DirectionBoar.UP].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - 20,this.y - this.game.camera.y- 35,2*PARAMS.SCALE);
                this.animations[2][this.facing].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - (this.width),this.y - this.game.camera.y - (this.height) ,2*PARAMS.SCALE);

            }
            else if(this.facing == DirectionBoar.DOWN){
                this.animations[2][this.facing].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - (this.width),this.y - this.game.camera.y - (this.height) ,2*PARAMS.SCALE);

                this.animations[1][DirectionBoar.DOWN].drawFrame(this.game.clockTick,ctx,this.x - this.game.camera.x - 20,this.y - this.game.camera.y + 15,2*PARAMS.SCALE);
        ...
        if (PARAMS.DEBUG) {
          ...
          ctx.strokeStyle = "Red";
          ctx.strokeRect(this.x - this.game.camera.x - (this.width), this.y - this.game.camera.y - (this.height), this.width*2,this.height*2);
